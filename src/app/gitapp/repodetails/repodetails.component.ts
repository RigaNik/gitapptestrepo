...
export class RepodetailsComponent implements OnInit {
    ...
    this._route.params.subscribe(
      (params) => {
        ...
        this._gitService.getRepo(owner, name).subscribe(item => {
          ...
          this.checkRepo(this.repoDetails);
        });
  ...
  private checkRepo(item: Repo) {
    console.log(item);
    if(window.localStorage.getItem('repo-' + item.fullname) != null) {
      this.saved = true;
    }
  }
  ...
  public saveRepo(repo: Repo) {
    if(!this.saved) {
      ...
      window.localStorage.removeItem('repo-' + repo.fullname);
      this.saved = false;
    }
  }
